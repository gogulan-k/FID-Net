...
MODEL_WEIGHTS = '../../FID-Net_modelWeights/jcoup_2dcon.h5'
...
def direct_decouple(model_weights, infile, outfile, shift = True, f1180  = False):
    ...
    data_ft = ft_second(data, npoints1=Hpoints, npoints2=Cpoints, f1180=False, shift = shift)
    ...
    res_ft = ft_second(res, npoints1=Hpoints, npoints2=Cpoints, f1180=False, shift = shift)
    ...
    plot_contour(ax1, data_ft,lvl=levy1, col1='red', col2 = 'teal', invert = False, invert_x = True, transpose = True)

    ax2 = plt.subplot(2,1,2)
    plot_contour(ax2, res_ft, lvl=levy2, col1='green', col2 = 'blueviolet', invert = False, invert_x = True, transpose = True)
...
def plot_contour(ax, ft_outer, col1 = 'blues', col2 = 'reds', lvl = None, invert = False, invert_x = False, transpose = True):
...
import argparse
parser = argparse.ArgumentParser(description='FID-Net 2D CON decoupling')
parser.add_argument('-in','--in', help='Input  spectra. This is a 2D in phase CON \
                    spectra with the 13C dimension in the time domain. The 13C dimension \
                    should be phased but the imaginary part retained and should not be \
                    apodized, zero-filled or Fourier transformed. \
                    The 15N dimension should be apodized, zero-filled, phased \
                    (the imaginary part deleted) then Fourier transformed. \
                    The order of the input dimensions must be 15N, 13C.', required=True)
parser.add_argument('-out','--out', help='Name of output spectra. Defaults to \
                        decouple.ft1', required=False, default = 'decouple.ft1')
args = vars(parser.parse_args())

direct_decouple(MODEL_WEIGHTS,args['in'],args['out'])
