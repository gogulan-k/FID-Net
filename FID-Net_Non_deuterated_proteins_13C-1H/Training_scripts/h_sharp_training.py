...
def process_example(pdic):
    ...
    fid2_jcoup =  tf.transpose(tf.multiply(
                    tf.transpose(tf.exp(
                        tf.tensordot(
                            tf.complex(
    ...
    # zero fill setup
    padding_2 = [[0,256-tf.shape(ft_r_jcoup)[0]],[0,0]]
    ...
    ft_orig_norm = tf.reshape(ft_orig_norm, [4,256,2])
...
def run_model(dtrain_file,dval_file):
    ...
    checkpoint_cb = keras.callbacks.ModelCheckpoint('h_sharpen_256.h5',
                                        save_best_only = True)
    ...
    with strategy.scope():
        ...
        print(model.summary())
        # model.load_weights('h_sharpen_256.h5')
...
TRAINSIZE = 500000
VALSIZE = 50000
TRAINING_SET = 'training_data/Hsharpen_small_train_500k.tfrecord'
VALIDATION_SET = 'training_data/Hsharpen_small_train_50k.tfrecord'
